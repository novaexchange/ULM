...
const char * const BITCOIN_CONF_FILENAME = "ulm.conf";
const char * const BITCOIN_PID_FILENAME = "ulmd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "ulm" is a composite category enabling all Ulm-related debug output
            if(ptrCategory->count(string("ulm"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\UlmCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\UlmCore
    // Mac: ~/Library/Application Support/UlmCore
    // Unix: ~/.ulmcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "UlmCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/UlmCore";
#else
    // Unix
    return pathRet / ".ulmcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty ulm.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override ulm.conf
